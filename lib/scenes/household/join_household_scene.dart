...
class _JoinHouseholdSceneState extends State<JoinHouseholdScene> {
  ...
  Widget build(BuildContext context) => AuthFrame(
        title: 'join_household'.tr,
        subtitle: 'join_household_subtitle'.tr,
        child: Observer(builder: (_) {
          if (store.error?.code == 'CLIENT_ALREADY_IN_HOUSEHOLD') {
            return FeatureBanner(message: 'already_in_household'.tr);
          }
  ...
  Widget _scanner() {
    if (permissionDenied) {
      return Column(children: [
        FeatureBanner(message: 'camera_permission_denied'.tr),
        const SizedBox(height: 16),
        AppButton(label: 'open_settings'.tr, onPressed: openAppSettings),
        AppButton(
            label: 'enter_code_manually'.tr,
          ...
          child: Stack(fit: StackFit.expand, children: [
            MobileScanner(
                ...
                errorBuilder: (_, __) => ColoredBox(
                    ...
                    child: Center(
                        child: Text('camera_unavailable'.tr,
  ...
  Widget _manualAndPreview() {
    ...
    return Column(children: [
      TextField(
        ...
        decoration:
            InputDecoration(labelText: 'invite_code'.tr, counterText: ''),
      ),
      const SizedBox(height: 16),
      if (state is LoadingState<InvitePreview>)
        const CircularProgressIndicator(),
      if (state is ErrorState<InvitePreview>) ...[
        FeatureBanner(message: errorCopy(state.error.code)),
        const SizedBox(height: 12),
        AppButton(label: 'preview_invite'.tr, onPressed: _preview),
      ],
  ...
  Widget _previewCard(InvitePreview preview) {
    ...
    return Column(children: [
      AppCard(
        ...
        Text('${'household_owner'.tr}: ${preview.ownerName}'),
        Text('${'member_count'.tr}: ${preview.memberCount}'),
        Text(
            '${'invite_expires'.tr}: ${MaterialLocalizations.of(context).formatMediumDate(preview.expiresAt.toLocal())}'),
      ])),
      if (guest) ...[
        const SizedBox(height: 12),
        TextField(
            ...
            decoration: InputDecoration(labelText: 'display_name_optional'.tr)),
      ],
      const SizedBox(height: 20),
      AppButton(
          label: 'confirm_join'.tr,
