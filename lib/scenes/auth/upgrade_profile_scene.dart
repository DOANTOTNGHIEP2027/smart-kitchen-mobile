...
class _UpgradeProfileSceneState extends State<UpgradeProfileScene> {
  ...
  Widget build(BuildContext context) => AuthFrame(
        ...
        child: Observer(builder: (_) => Column(children: [
              ...
              TextField(controller: email, enabled: !store.isSubmitting, keyboardType: TextInputType.emailAddress, decoration: InputDecoration(labelText: 'email'.localized(context), errorText: _fieldError('email'))),
              const SizedBox(height: 12),
              TextField(controller: password, enabled: !store.isSubmitting, obscureText: true, decoration: InputDecoration(labelText: 'password'.localized(context), errorText: _fieldError('password'))),
              const SizedBox(height: 12),
              TextField(controller: fullName, enabled: !store.isSubmitting, textCapitalization: TextCapitalization.words, decoration: InputDecoration(labelText: 'full_name_optional'.localized(context), errorText: _fieldError('fullName'))),
