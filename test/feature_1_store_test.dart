...
import 'package:smart_kitchen_mobile/domain/user_summary.dart';
...
void main() {
  ...
  group('AuthStore', () {
    test('client validation prevents register request', () async {
      ...
      final store = AuthStore(api, SessionStore(_MemoryTokenStorage()), const UnavailableGoogleAuthGateway());
    ...
    test('maps bad credentials to one non-enumerating field error', () async {
      final api = _FakeAuthApi()..loginError = const AuthExpiredException('ERR_AUTH_004', 'bad');
      final store = AuthStore(api, SessionStore(_MemoryTokenStorage()), const UnavailableGoogleAuthGateway());
  ...
  group('HouseholdStore', () {
    test('normalizes code and maps preview success', () async {
      ...
      final store = HouseholdStore(householdApi, authApi, SessionStore(_MemoryTokenStorage()));
    ...
    test('member join silently refreshes household claims', () async {
      ...
      session.configureRefresh((_) async => TokenPair(
            accessToken: _jwt({'household_id': 'new-household', 'role': 'MEMBER', 'provider': 'EMAIL'}),
  ...
  test('OTP rejects non-six-digit input without an API request and disposes timer', () async {
    final api = _FakeAuthApi();
    final store = OtpStore(api, SessionStore(_MemoryTokenStorage()), email: 'a@example.com', flow: OtpFlow.register);
...
class _FakeAuthApi implements AuthApi {
  ...
  Future<RegisterResult> register({required String email, required String password, required String fullName}) async {
    ...
    if (registerFailures-- > 0) throw const NetworkException();
